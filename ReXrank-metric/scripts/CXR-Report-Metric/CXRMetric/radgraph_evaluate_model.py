...
def generate_radgraph(model_path, raw_path, output_path, cuda=-1,
    ...
    # Try using radgraph native API first (avoids allennlp CLI issues)
    try:
        from radgraph import RadGraph
        import pandas as pd
        import tarfile

        print("Using radgraph native API (no allennlp CLI needed)...")

        # Prepare local model if needed
        # RadGraph expects: model_cache_dir/radgraph/ with config.json, vocabulary/, etc.
        local_model_cache = "/mnt/shared-storage-user/ai4good1-share/xieyuejin/models/RRG_scorers"
        model_dir = os.path.join(local_model_cache, "radgraph")

        # Extract local radgraph.tar.gz if not already extracted
        if not os.path.exists(model_dir) or not os.listdir(model_dir):
            print(f"Extracting local model to {model_dir}...")
        ...
        radgraph_model = RadGraph(
            ...
            cuda=cuda,
            batch_size=8,
            model_cache_dir=local_model_cache,  # Point to parent dir of radgraph/
            tokenizer_cache_dir="/mnt/shared-storage-user/ai4good1-share/hf_hub"  # Use local HF cache
        )
        ...
        # Run inference
        results = radgraph_model(texts)

        # Format output in expected format
        # results is a dict with keys as string indices: {'0': {...}, '1': {...}, ...}
        final_dict = {}
        for idx, study_id in enumerate(study_ids):
            idx_key = str(idx)
            if idx_key in results:
                result = results[idx_key]
                final_dict[str(study_id)] = {
        ...
        print(f"✅ RadGraph inference complete using native API")
        return

    except Exception as e:
        print(f"⚠️  Native API failed: {e}")
        import traceback
        traceback.print_exc()
        print("Falling back to original inference method...")

    # Original inference method (requires allennlp CLI)
...
def run_radgraph(gt_path, pred_path, out_dir, radgraph_model_checkpoint,
    ...
    # Use GPU 0 (cuda=0) instead of CPU (cuda=-1)
    import torch
    cuda_device = 0 if torch.cuda.is_available() else -1
    generate_radgraph(radgraph_model_checkpoint, gt_path, gt_out_path, cuda=cuda_device)
    generate_radgraph(radgraph_model_checkpoint, pred_path, pred_out_path, cuda=cuda_device)
