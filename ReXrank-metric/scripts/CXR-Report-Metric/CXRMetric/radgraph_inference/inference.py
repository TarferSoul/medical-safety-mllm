...
def run_inference(model_path, cuda):
    ...
    # Try using Python API first (avoids CLI dependency issues)
    try:
        from .inference_no_cli import run_inference_with_fallback
        run_inference_with_fallback(model_path, data_path, out_path, cuda)
    except:
