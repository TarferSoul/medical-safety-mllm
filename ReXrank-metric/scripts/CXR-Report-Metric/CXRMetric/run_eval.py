...
def add_bertscore_col(gt_df, pred_df, use_idf):
    ...
    # Use local absolute path to avoid downloading from HuggingFace Hub
    # Must specify num_layers=5 (optimal for distilroberta-base) when using local path
    # Must also specify baseline_path manually when using local path
    model_path = "/mnt/shared-storage-gpfs2/gpfs2-shared-public/huggingface/zskj-hub/models--distilbert--distilroberta-base"

    # Find bert_score package location to get baseline file
    import bert_score
    ...
    scorer = BERTScorer(
        ...
        num_layers=5,  # distilroberta-base 的最佳层数
        batch_size=256,
        lang="en",
        rescale_with_baseline=True,
        baseline_path=baseline_path,  # 手动指定 baseline 路径
        idf=use_idf,
