...
class PretrainedTransformerIndexer(TokenIndexer):
    ...
    def _postprocess_output(self, output: IndexedTokenList) -> IndexedTokenList:
        ...
        if self._max_length is not None:
            ...
            # Compatible with transformers 4.36+ where build_inputs_with_special_tokens was removed
            if hasattr(self._tokenizer, 'build_inputs_with_special_tokens'):
                folded_indices = [
                    self._tokenizer.build_inputs_with_special_tokens(segment)
                    for segment in folded_indices
                ]
            else:
                # Manual implementation: [CLS] + segment + [SEP]
                folded_indices = [
                    [self._tokenizer.cls_token_id] + segment + [self._tokenizer.sep_token_id]
                    for segment in folded_indices
                ]
            # Flattens
