...
def get(
    ...
    """
    # Map model name to local path if available
    local_model_map = {
        'microsoft/BiomedNLP-PubMedBERT-base-uncased-abstract-fulltext':
            '/mnt/shared-storage-user/ai4good1-share/hf_hub/BiomedNLP-PubMedBERT-base-uncased-abstract-fulltext'
    }
    model_path = local_model_map.get(model_name, model_name)

    global _model_cache
    spec = TransformerSpec(model_path, override_weights_file, override_weights_strip_prefix)
    ...
    if transformer is None:
        if override_weights_file is not None:
            ...
            transformer = AutoModel.from_pretrained(model_path, state_dict=override_weights, local_files_only=True)
        else:
...
def get_tokenizer(model_name: str, **kwargs) -> transformers.PreTrainedTokenizer:
    ...
    # Use local path if model is in map, otherwise use original model name
    model_path = local_model_map.get(model_name, model_name)

    # Force offline mode to prevent network access
    kwargs['local_files_only'] = True

    cache_key = (model_path, frozenset(kwargs.items()))  # Use model_path for cache key
    global _tokenizer_cache
    tokenizer = _tokenizer_cache.get(cache_key, None)
    if tokenizer is None:
        tokenizer = transformers.AutoTokenizer.from_pretrained(model_path, **kwargs)
