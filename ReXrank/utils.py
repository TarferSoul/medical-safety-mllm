...
def seg_2d_process(image_path, pred_mask, img_size=224):
    ...
    if pred_mask.sum() != 0:
        ...
        binary_array = cv2.resize(
            pred_mask, (image.shape[1], image.shape[0]), interpolation=cv2.INTER_NEAREST
        )
    ...
    if seg_mask.sum() != 0:
        ...
        for i in range(seg_mask.shape[0]):
            ...
            contours, _ = cv2.findContours(
                slice_mask, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE
            )
            ...
            if contours:
                ...
                binary_array = (
                    cv2.drawContours(
                        binary_array, contours, -1, 255, thickness=cv2.FILLED
                    )
                    / 255
                )
                binary_array = cv2.resize(
                    binary_array, slice_img.shape, interpolation=cv2.INTER_NEAREST
                )
...
def read_image(image_path, modality, task):
    if image_path.endswith((".jpg", ".jpeg", ".png")):
        return load_and_preprocess_image(
            Image.open(image_path).convert("RGB"), modality, task
        )
...
def generate(
    ...
    if task.lower() == "report" or (
        modal.lower() == "derm" and task.lower() == "classification"
    ):
        if len(context) == 0 and task.lower() == "report":
            context = "Comparison:None."
        prompt = "<context>" + context + "</context>" + prompt
    img_embeds, atts_img, img_embeds_list = model.encode_img(
        image.unsqueeze(0), modals=[modal.lower()], task_types=[task.lower()]
    )
    if task.lower() == "vqa":
    ...
    prefix = "###Human:" + "".join(
        [f"<img{i}>" + "".join(placeholder) + f"</img{i}>" for i in range(num_imgs)]
    )
    img_embeds, atts_img = model.prompt_wrap(img_embeds, atts_img, [prefix], [num_imgs])
    prompt += "###Assistant:"
    prompt_tokens = model.llama_tokenizer(
        prompt, return_tensors="pt", add_special_tokens=False
    ).to(image.device)
    new_img_embeds, new_atts_img = model.prompt_concat(
        img_embeds, atts_img, prompt_tokens
    )
